...
namespace Ryujinx.HLE.HOS.Services.Am.AppletAE.AllSystemAppletProxiesService.SystemAppletProxy
{
    class ILibraryRealAppletAccessor : DisposableIpcService
    {
        ...
        public RealApplet _applet => System.RealAppletManager.GetApplet(ProgramId);
        public ILibraryRealAppletAccessor(ulong programId, ulong currentId, Horizon system)
        {
            ...
            var applet = new RealApplet();
            applet.BeforeId = currentId;
            applet.LastRenderLayer = system.SurfaceFlinger.RenderLayerId;
            applet.AppletId = AppletIndexMap.GetAppletId(programId);
            applet._stateChangedEvent = _stateChangedEvent;
            if (AppletIndexMap.IsPartialForeground(programId))
            {
                applet.LibraryAppletMode = LibraryAppletMode.PartialForeground;
            }
            else
            {
                applet.LibraryAppletMode = LibraryAppletMode.AllForegroundInitiallyHidden;
            }
            system.RealAppletManager.RegisterApplet(programId, applet);
            System = system;
            ProgramId = programId;
            if (AppletIndexMap.ShouldProvideCommonArguments(programId))
            {
                Logger.Info?.Print(LogClass.Service,$"Common arguments provided for program {programId}");
                ...
                _applet.InputBuffer.Enqueue(argumentsBytes);
            }
        }
        ...
        {
            string path = context.Device.Configuration.ContentManager.GetInstalledContentPath(ProgramId, StorageId.BuiltInSystem,
                NcaContentType.Program);
            path = VirtualFileSystem.SwitchPathToSystemPath(path);
            _applet.AppletManagerBefore = context.Device.System.AppletState;
            context.Device.System.CreateNewAppletManager();
            context.Device.Processes.LoadNca(path);
            return ResultCode.Success;
        }
        ...
        public ResultCode PushInData(ServiceCtx context)
        {
            ...
            _applet.InputBuffer.Enqueue(data.Data);
        ...
        {
            if (_applet.InputBuffer.TryDequeue(out byte[] data))
            {
        ...
        public ResultCode PushInteractiveInData(ServiceCtx context)
        {
            ...
            _applet.InteractiveBuffer.Enqueue(data.Data);
        ...
        {
            if (_applet.InteractiveBuffer.TryDequeue(out byte[] data))
            {
                MakeObject(context, new IStorage(data));
